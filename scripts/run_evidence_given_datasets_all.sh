...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/evidence_given_datasets_all_$(date +%Y%m%d_%H%M%S).log"

exec > >(tee -a "$LOG") 2>&1

echo "==== EVIDENCE-GIVEN DATASETS ALL START ===="
date
echo "PROJECT_DIR=$PROJECT_DIR"
echo "LOG=$LOG"

echo ""
echo "==== Step 1: Export/normalize datasets with loader env ===="
source "$(conda info --base)/etc/profile.d/conda.sh"
conda activate rcv_fever_loader
python --version

mkdir -p scripts data/processed/evidence_given

cat > scripts/export_evidence_given_datasets.py <<'PY'
import json
import re
import random
...
python scripts/export_evidence_given_datasets.py

echo ""
echo "==== Step 2: Run evidence-given NLI/risk experiments in main env ===="
conda activate rcv_py310
source scripts/select_free_gpu.sh 1
python --version

cat > scripts/run_evidence_given_verification.py <<'PY'
import os
import json
import gc
import random
...
python scripts/run_evidence_given_verification.py

echo ""
echo "==== Final evidence-given files ===="
find outputs/metrics/evidence_given outputs/tables/evidence_given outputs/final_report outputs/predictions/evidence_given -maxdepth 1 -type f | sort | tail -n 120 || true

echo ""
echo "==== EVIDENCE-GIVEN DATASETS ALL END ===="
date

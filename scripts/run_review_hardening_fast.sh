...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/review_hardening_fast_$(date +%Y%m%d_%H%M%S).log"

exec > >(tee -a "$LOG") 2>&1

echo "==== REVIEW HARDENING FAST START ===="
date
...
mkdir -p outputs/final_report outputs/metrics/review_hardening outputs/tables/review_hardening outputs/latex_tables outputs/figures scripts

cat > scripts/run_review_hardening_fast.py <<'PY'
import json
...
python scripts/run_review_hardening_fast.py

echo ""
echo "==== Final fast hardening files ===="
find outputs/final_report outputs/tables/review_hardening outputs/metrics/review_hardening outputs/latex_tables outputs/figures \
  -maxdepth 1 -type f | grep -E "fast|ablation|oracle|risk_coverage|utility_risk" | sort || true

echo ""
echo "==== REVIEW HARDENING FAST END ===="
date

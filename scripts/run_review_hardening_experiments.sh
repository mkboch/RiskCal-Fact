...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/review_hardening_experiments_$(date +%Y%m%d_%H%M%S).log"

exec > >(tee -a "$LOG") 2>&1

echo "==== REVIEW HARDENING EXPERIMENTS START ===="
date
...
echo ""
echo "==== Select dynamic GPU ===="
source scripts/select_free_gpu.sh 1 || true

echo ""
echo "==== Write review-hardening script ===="
cat > scripts/run_review_hardening_experiments.py <<'PY'
import os
import re
import gc
import json
...
echo ""
echo "==== Run review-hardening experiments ===="
python scripts/run_review_hardening_experiments.py || echo "WARNING: review-hardening script returned nonzero status."

echo ""
echo "==== Final review-hardening files ===="
find outputs/final_report outputs/tables/review_hardening outputs/metrics/review_hardening outputs/latex_tables outputs/figures outputs/predictions/review_hardening \
  -maxdepth 1 -type f | grep -E "review_hardening|ablation|oracle|risk_coverage_fever_full_dev|risk_coverage_vitaminc|utility_risk_fever_full_dev|utility_risk_vitaminc|ci" | sort || true

echo ""
echo "==== REVIEW HARDENING EXPERIMENTS END ===="
date

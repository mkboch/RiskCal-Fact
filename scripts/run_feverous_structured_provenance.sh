...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/feverous_structured_provenance_$(date +%Y%m%d_%H%M%S).log"

exec > >(tee -a "$LOG") 2>&1

echo "==== FEVEROUS STRUCTURED PROVENANCE START ===="
date
...
mkdir -p scripts data/processed/feverous outputs/metrics/feverous outputs/tables/feverous outputs/final_report outputs/latex_tables outputs/figures

cat > scripts/run_feverous_structured_provenance.py <<'PY'
import json
import re
...
python scripts/run_feverous_structured_provenance.py

echo ""
echo "==== Update final project status ===="
cat > scripts/update_project_ready_status.py <<'PY'
import json
from pathlib import Path
...
python scripts/update_project_ready_status.py

echo ""
echo "==== Final FEVEROUS/project files ===="
find outputs/final_report outputs/tables/feverous outputs/metrics/feverous outputs/latex_tables outputs/figures data/processed/feverous -maxdepth 1 -type f | sort | tail -n 160

echo ""
echo "==== FEVEROUS STRUCTURED PROVENANCE END ===="
date

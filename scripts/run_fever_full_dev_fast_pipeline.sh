...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/fever_full_dev_fast_pipeline_$(date +%Y%m%d_%H%M%S).log"

exec > >(tee -a "$LOG") 2>&1

echo "==== FEVER FULL-DEV FAST PIPELINE START ===="
date
...
echo ""
echo "==== Write fast FEVER full-dev script ===="
cat > scripts/run_fever_full_dev_fast.py <<'PY'
import os
import re
import gc
import json
...
echo ""
echo "==== Run fast FEVER full-dev script ===="
python scripts/run_fever_full_dev_fast.py || echo "WARNING: fast FEVER full-dev script ended with an error."

echo ""
echo "==== Make final summary after fast FEVER full-dev ===="
python scripts/make_all_final_paper_tables.py || true

echo ""
echo "==== Final fast full-dev files ===="
find outputs/metrics/fever outputs/tables/fever outputs/final_report outputs/predictions/fever -maxdepth 1 -type f | grep -E "fast_full_dev|fever_fast|paper_ready" | sort || true

echo ""
echo "==== FEVER FULL-DEV FAST PIPELINE END ===="
date
